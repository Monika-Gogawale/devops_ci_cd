				...
				steps {
					...
					checkout(
						[
							...
							userRemoteConfigs: [[url: 'https://github.com/Monika-Gogawale/devops_ci_cd.git']]						
						]
						)
	...
	stage('Deploying on Docker') {
		...
		steps {
			script {
				if (env.BRANCH_NAME == 'master'){
				sh """
				if [ `curl -o /dev/null -s -w "%{http_code}\n" http://100.26.214.73:83/index.html` = 200 ];
				then
				...
				sh """
				if [ `curl -o /dev/null -s -w "%{http_code}\n" http://100.26.214.73:82/index.html` = 200 ];
				then
				echo "Merge Successful"
				else
				echo "Merge Failed"
	...
	post {
		success {
			...
			if (env.BRANCH_NAME == 'master'){
				...
				sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/Monika-Gogawale/devops_ci_cd.git HEAD:master')
					}
				} else if (env.BRANCH_NAME == 'dev'){
